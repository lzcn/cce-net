...
@torch.no_grad()
def eval(net: models.ContextTripletNet, loader: DataLoader):
    ...
    for data in tqdm(loader, desc="Evaluating Recommendation AUC"):
        ...
        scores = net(data.x, data.target, data.edge_index, data.num_pairs, data.item_mask)
        y_pred += scores
